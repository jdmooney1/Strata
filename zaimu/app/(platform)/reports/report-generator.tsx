...
type GenState =
  | { status: "idle" }
  | { status: "generating" }
  | { status: "done"; title: string; overallStatus: string; reportId?: string }
...
export function ReportGenerator({ orgId }: { orgId: string }) {
  ...
  // Load assets from DB when the panel opens
  useEffect(() => {
    if (!open || assets.length > 0) return;
    fetch(`/api/assets?orgId=${encodeURIComponent(orgId)}`)
      .then((r) => r.json())
      .then((data: { assets?: { id: string; name: string }[] }) => {
        const list = (data.assets ?? []).map((a) => ({ id: a.id, name: a.name }));
        setAssets(list);
        if (list.length > 0 && !assetId) setAssetId(list[0].id);
      })
      .catch(() => {
        // Fallback to known seed asset if API fails
        setAssets([{ id: "asset_collins_001", name: "Collins Square Tower 5" }]);
        setAssetId("asset_collins_001");
      });
  }, [open, orgId, assets.length, assetId]);
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const data = (await res.json()) as {
        id?: string;
        content?: { title?: string; overallStatus?: string };
      };
      setState({
        status: "done",
        title: data.content?.title ?? "Report generated",
        overallStatus: data.content?.overallStatus ?? "GREEN",
        reportId: data.id,
      });
      // Refresh the report archive list
      router.refresh();
    } catch (err) {
      setState({
        status: "error",
        ...
        <div className="data-card-body border-t border-[var(--color-border)] space-y-4">
          ...
          {state.status === "done" && (
            <div className="border border-[var(--color-status-green)] bg-[var(--color-status-green-bg)] rounded-lg p-4 flex items-start gap-3">
              ...
              <button
                ...
                className="text-xs text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] ml-auto"
              >
                Generate another
              </button>
            </div>
          ...
          {state.status === "error" && (
            <div className="border border-[var(--color-status-red)] bg-[var(--color-status-red-bg)] rounded-lg p-4 flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-[var(--color-status-red)]">
                  Generation failed
                </p>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div>
                ...
                <select
                  ...
                  className="w-full text-sm border border-[var(--color-border)] rounded-md px-3 py-2 bg-white focus:outline-none focus:border-[var(--color-navy-400)] disabled:opacity-50"
                >
                  {assets.length === 0 ? (
                    ...
                    <button
                      ...
                      className={`flex-1 text-xs font-medium py-2 rounded-md border transition-colors ${
                        language === v
                          ...
                          : "border-[var(--color-border)] bg-white text-[var(--color-text-secondary)] hover:bg-[var(--color-slate-50)]"
                      }`}
                    >
                      {l}
                    </button>
          ...
          {state.status !== "done" && (
            <div className="flex items-center gap-3">
              <button
                ...
                disabled={state.status === "generating" || !assetId}
